...
Hard rule from the tool policy: all artwork and all lettering is rendered by imagegen. Scripts are only for mechanical operations such as compositing the unmodified `series-logo.png` file or checking dimensions — never for drawing text or content.
...
- Text: avoid extra small text. Only include subtitle or supporting words when requested or already part of the approved logo, and render any such text with imagegen in the series lettering style — never with a script or system font.
...
After generation, composite the approved `series-logo.png` file as an overlay if the tool did not preserve it exactly — this exact-file placement is the one operation a script may do. If the generated image includes a distorted, misspelled, or alternate logo, regenerate or edit it out with imagegen before placing the approved logo.
...
- Confirm the cover establishes reusable motifs without depicting one specific lesson moment (unless the series requires it).
